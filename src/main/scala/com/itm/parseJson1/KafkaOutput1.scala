...
object KafkaOutput1 {
  def main(args: Array[String]) {
    ...
    val apibb :TssRuleParserImpl = new TssRuleParserImpl()

    val ssd : Broadcast[TssRuleParserImpl] = ssc.sparkContext.broadcast(apibb)
    ...
    lineDStreamMessage.foreachRDD(rdd => {
      rdd.foreachPartition(iter => {
        ...
        iter.foreach (
            ...
            //  println(record)
              println("------------- end parse ----------------")
              KafkaProducer.sendMessages(topicBroadcast.value,record,brokerListBroadcast.value)
            }
          )
      })
    })
